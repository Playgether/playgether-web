...
export const ResponsiveIntervalFunctionComponent = () => {
    ...
    return (
        <div className="bg-white-300 flex-1 text-black-400 relative right-0">
            {GlobalMessages.map((message, index)=>
                <div className={`w-full h-16 flex flex-row items-center gap-2  ${index === currentIndex ? '':'hidden'}`} key={message.id}>
                    <TextLimitComponent 
                        text={message.username + ':'}
                        maxCharacters={40}
                        className={`relative z-10 text-xs flex items-center justify-center -ml-4 ${index === currentIndex ? 'animate-fadeIn':'animate-fadeOut'}`}
