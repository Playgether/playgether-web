...
import TextLimitComponentFunction from "./TextLimitComponentFunction";

export interface TextLimitComponentProps
...
const TextLimitComponent = ({
  ...
  textFinal = "...",
  ...rest
}: TextLimitComponentProps) => {
  const limitedText = TextLimitComponentFunction(text, maxCharacters);

  return (
    <div className={twJoin(rest.className)}>
      <p className={twJoin("whitespace-pre-wrap", paragraphClassName)}>
        {limitedText}
        {text.length > maxCharacters && (
          <span className={textFinalClassName}>{textFinal}</span>
        )}
      </p>
    </div>
  );
