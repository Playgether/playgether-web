...
async function handler(
  ...
  try {
    const response = await api[method](`/api/v1/profiles/${pk}/mute/`, method === "post" ? {} : undefined, {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    return NextResponse.json(response.data, { status: response.status });
  } catch (error: any) {
    const status = error.response?.status ?? 500;
    const detail = error.response?.data?.detail ?? "Erro ao processar ação de silenciar";
