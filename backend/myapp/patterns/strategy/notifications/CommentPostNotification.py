...
class CommentPostNotification(NotificationInterface):
    ...
    def generate_notification(self, instance, classNotification):
        if instance.content_object.created_by_user.id == instance.user.id:
            ...
            classNotification.objects.create(user=instance.content_object.created_by_user, message = f'{instance.user.first_name} comentou sua postagem: {instance.comment}', content_type = content_type, object_id = instance.id)
            return
